...
import { rejectLeave } from "@/modules/leave/services/leave"
...
export async function rejectLeaveAction(
  ...
  try {
    const leave = await rejectLeave(parsed.data.id)
    ...
    return {
      ok: false,
      error: "Unable to reject this leave request. Please try again.",
    }
  }
}
